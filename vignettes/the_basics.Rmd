...
The package also supports rendering directly from (R)markdown to HTML directly. This functionality uses functionality built directly into the rmarkdown package. Using the file, 'joss.Rmd' in the examples from the package, we can directly compile this file from R Markdown directly to HTML with the following `highlight_html` arugments.
