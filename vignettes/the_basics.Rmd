...
Have you ever wanted to include some form of highlighting, either conditional or nonconditional, when going from a (R)markdown input document to a html output document? If so, this package may be worthwhile to you. The `highlightHTML` package inject CSS into the output HTML document automatically via ids (more information on CSS ids here: <http://www.cssbasics.com/css-ids/>). 
...
You could then add formatting by adding the following tags to the table, again this was added by hand, but a conditional example with R is shown later.
...
# Formatting Text
Formatting text is also simple in a markdown document. Braces `{}` are used to identify text that should be formatted. An id tag is then added to link this to the CSS formatting. 

For example, perhaps there is `{#colgold some text}` to turn gold. In the previous example, the text "some text" would be processed to be formatted using the CSS id #colgold. 

Another example file can be run with the following commands to show more text formatting examples:
```{r text, eval = FALSE}
...
I believe a more typical workflow for the highlightHTML package is to go from an (R)markdown document and compile it directly with the `highlight_html` function to create the output HTML file. This likely represents a larger audience compared to editing raw HTML directly to add the CSS ids.

Another benefit of this workflow, is the ability to use the `table_id_inject` function. This function allows the injection of CSS ids into summary tables created within R using conditional expressions. 
